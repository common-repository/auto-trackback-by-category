<?php
/*
Plugin Name: Auto-TrackBack by Category
Plugin URI: http://www.texas-fight.com
...
Version: 1.0
Author: Paul Young
Author URI: http://www.texas-fight.com
*/
	...
	if ($categories) {
		foreach ($categories as $category) {
			if ($category->category_parent == $parent) {
				...
				$cat_tracks = $wpdb->get_results("SELECT * FROM wp_cat_tracks WHERE cat_ID = $category->cat_ID");
				$cat_tracks = $cat_tracks[0];
				$pad = str_repeat('&#8212; ', $level);
				
				if ( $user_level > 3 )
					$edit = "<input type='text' name='trackback_uri[$category->cat_ID]' value='$cat_tracks->uri' style='width: 360px' /><br />\n";
				else
					$edit = '';
				
				$class = ('alternate' == $class) ? '' : 'alternate';
				
				echo "<tr class='$class'><th scope='row'>$category->cat_ID</th><td>$pad $category->cat_name</td><td>$edit</td></tr>";
...
function atc_cat_tracks_edit() {
  ...
  foreach ($_POST['trackback_uri'] as $cat_ID => $uri_proper) {					
		$wpdb->query("REPLACE INTO wp_cat_tracks SET cat_ID=$cat_ID, uri='$uri_proper'");
		}
}
...
function atc_cat_tracks_backfill($post_ID) {	
	...
	foreach($atc_post_cats as $cat_ID) {		
		$auto_uri .= $wpdb->get_var("SELECT uri FROM wp_cat_tracks WHERE cat_ID=$cat_ID") . " ";			
	}		
